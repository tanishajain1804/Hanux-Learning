...
# Extract frames between index 150 and 390
for i, frame_idx in enumerate(range(150, 390, 30)):
    ...
    if ret:
        ...
        out_path = os.path.join(output_dir, f"mid_scroll_{i}.jpg")
        cv2.imwrite(out_path, crop)
        print(f"Saved mid scroll frame {i} (frame index {frame_idx}) to {out_path}")
    else:
